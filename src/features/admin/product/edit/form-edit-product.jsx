...
import { updateProduct } from "@/actions/admin/product/product";
...
export function FormEditProduct({ data, typeProducts }) {
  ...
  useEffect(() => {
    if (data) {
      ...
      form.reset({
        ...
        is_check_username:
          data.is_check_username === 1 || data.is_check_username === true,
      ...
      if (data.image_thumbnail) {
        setThumbnailPrev(data.image_thumbnail);
      }
      if (data.image_cover) {
        setCoverPrev(data.image_cover);
      }

      // Parse dan set data_input
      if (data.data_input) {
        try {
          const parsedDataInput =
            typeof data.data_input === "string"
              ? JSON.parse(data.data_input)
              : data.data_input;

          if (Array.isArray(parsedDataInput) && parsedDataInput.length > 0) {
            setDataInputs(parsedDataInput);
          }
        } catch (error) {
          console.error("Error parsing data_input:", error);
        }
      }

      // Parse dan set type_data_product
      if (data.type_data_product) {
        try {
          const parsedTypeData =
            typeof data.type_data_product === "string"
              ? JSON.parse(data.type_data_product)
              : data.type_data_product;

          if (Array.isArray(parsedTypeData) && parsedTypeData.length > 0) {
            // Tambahkan id jika belum ada
            const withIds = parsedTypeData.map((item) => ({
              ...item,
              id: item.id || generateUUID(),
            }));
            setTypeDataProducts(withIds);
          }
        } catch (error) {
        ...
        <Form {...form}>
          <div className="space-y-6">
            ...
            <Button
              ...
              disabled={
                updateMutation.isPending || thumbnailLoading || coverLoading
              }
