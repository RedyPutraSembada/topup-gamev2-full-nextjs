...
import { updateDataImageSlideShow } from '@/actions/admin/images-slideshow/images-slideshow';
...
export function FormEditImagesSlideShow({ data }) {
  ...
  // Parse data_slider dari JSON string ke array
  useEffect(() => {
    if (data?.data_slider) {
      try {
        const parsedSlides = JSON.parse(data.data_slider);
        setSlides(parsedSlides);
      } catch (error) {
        console.error('Error parsing slides:', error);
        toast.error('Gagal memuat data slideshow');
        setSlides([{ id: Date.now(), title: '', image: '', desc: '', order: 1 }]);
      }
    }
  }, [data]);

  const updateMutation = useMutation({
    mutationFn: updateDataImageSlideShow,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["images-slideshow"] });
      toast.success("Data images slideshow updated successfully");
      ...
      <div className="container mx-auto p-4 sm:p-6 lg:p-10">
        <div className="max-w-5xl mx-auto">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold mb-2">Edit Slideshow</h1>
              <p className="text-gray-400 text-sm">Edit dan atur slide untuk homepage</p>
            </div>
            <button
              type="button"
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-col sm:flex-row gap-3 pt-4">
                ...
                <button
                  type="button"
                  onClick={onSubmit}
                  disabled={updateMutation.isPending || isAnyImageUploading}
